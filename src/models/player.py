...
class PacMan(Entity):
    ...
    def _get_next_pos(self, d: int, grid_width: int, grid_height: int, cx: int, cy: int) -> tuple[int, int]:
        cx %= grid_width
        cy %= grid_height
        if d == Direction.UP: cy -= 1
        elif d == Direction.DOWN: cy += 1
        elif d == Direction.LEFT: cx -= 1
        elif d == Direction.RIGHT: cx += 1
